...
function getSecondsRequired(N, F, P) {
    function jumpTo(frog, nextPad, allPads) {
      allPads[nextPad] = 1;
      allPads[frog] = 0;
      return true;
    }

    let pads = Array(N).fill(0);
    ...
    while (jumping) {
      console.log(pads);
      nextPad = pads.indexOf(0, min);
      jumping = nextPad < max ? jumpTo(min, nextPad, pads) : false;
      min = pads.indexOf(1);
      tick = jumping ? tick + 1 : tick;
    }
    return N - max + (F - 1) + tick;
}

console.log(getSecondsRequired(20, 3, [5, 2, 4]));
